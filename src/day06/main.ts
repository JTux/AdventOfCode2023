...
const example = getExample("day06");
const input = getInput("day06");

const file = input;

const lines = file.split(EOL);
const dataSet = lines.map(l => l.split(" ").filter(s => s).slice(1));

const races: { raceTime: number, recordDistance: number }[] = [];
for (let i = 0; i < dataSet[0].length; i++) {
  races.push({
    raceTime: Number(dataSet[0][i]),
    recordDistance: Number(dataSet[1][i])
  });
}
...
const getRaceData = (r: { raceTime: number, recordDistance: number }) => {
  ...
  for (let i = 0; i <= r.raceTime / 2; i++) {
    let timeHeld = i;
    let remainingTime = r.raceTime - timeHeld;
    let speedPerMs = i;
